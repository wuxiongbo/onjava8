...
/**
 * Strategy提供了接口，功能是通过其中唯一的 {@link Strategy#approach(String)} 方法来承载的。
 ...
 * [4] 这是Java 8的方法引用，突出的特点是 双冒号::
 *     ::的左边是 类名 或 对象名，右边是 方法名，但是没有参数列表。
 * [5] 在使用了默认的Soft策略之后，我们遍历数组中的所有策略，并使用 {@link Strategize#changeStrategy(Strategy)} 方法将每个策略放入 {@linkplain Strategize} 中。
 * [6] 现在，每次调用 {@link Strategize#communicate()} 都会产生不同的行为，这取决于此时所使用的 策略“代码对象”。我们传递了‘行为’，而不只是传递’数据‘。（有时，函数式语言将其描述为“代码即数据”）
 * @author 10027088
 */
